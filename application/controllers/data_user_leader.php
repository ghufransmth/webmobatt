...
class Data_user_leader extends CI_Controller {
...
$this->load->model('master_model_user_leader');
$this->load->model('master_model_get_data');

		if($this->session->userdata('status_login_hmcs')){
	...
	public function index()
	{
		...
		$this->load->view('page/data_user_leader/index.php');
		$this->load->view('footer.php');
	}
	public function get_master_data_pegawai($id_user=false){
			$data['data']=$this->master_model_user_leader->data_master_pegawai($id_user);
			for($i=0;$i<count($data['data']);$i++){
				...
				$data['data'][$i]['foto']="<img src='".base_url()."/upload/foto_pegawai/".$data['data'][$i]['foto']."' style='width:100px;height:100px'>";
			}
			$this->output->set_content_type('application/json')->set_output(json_encode($data));	
	}
	public function input_data_user_leader()
	{
				
		$this->load->view('style.php');
		$this->load->view('menu_header.php');
		$this->load->view('page/data_user_leader/input_user_leader.php');
		$this->load->view('footer.php');
	}
		...
		public function save_pegawai()
	{
		...
		$foto['nama_file'] 		= pathinfo($_FILES['nama_file']['name'], 	PATHINFO_FILENAME);
			if ($foto['nama_file'] !='') {
			$nama_file 			= $this->master_model_user_leader->upload_foto('nama_file', $_FILES['nama_file']['name']);
			$options['nama_file']  = substr($nama_file, 0, -4);	
			}
			else{
				unset($foto['nama_file']);
			}
			$data_devisi_get=$this->master_model_user_leader->save($data_devisi);
			if($data_devisi_get){
	...
	public function view_edit_data_pegawai($id_user=false)
	{
	$data['data']=$this->master_model_user_leader->data_master_pegawai($id_user);
			for($i=0;$i<count($data['data']);$i++){
				$data['data'][$i]['data_pangkat']=$this->master_model_get_data->data_pangkat($data['data'][$i]['pangkat']);
		...
		$this->load->view('style.php');
		$this->load->view('menu_header.php');
		$this->load->view('page/data_pegawai/edit_data_pegawai.php',$data);
		$this->load->view('footer.php');
		//$this->output->set_content_type('application/json')->set_output(json_encode($data));
		...
		public function edit_pegawai()
	{
		...
		$foto['nama_file'] 		= pathinfo($_FILES['nama_file']['name'], 	PATHINFO_FILENAME);
			...
			$data_devisi_get=$this->master_model_user_leader->edit($data_devisi);
			if($data_devisi_get){
				$data['result']="Data Successfully Deleted";
				$data['status']	= true;
				$data['is_logged_in']=true;
	...
	public function delete_data_pegawai($id_devisi=false)
	{
		$data_devisi=$this->master_model_user_leader->delete_data_pegawai($id_devisi);
			if($data_devisi){
