...
class Delegated extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('page/delegated/index.php',$data);
	...
	public function approval_absensi($id_devisi=false)
	{
		...
		public function save_delegated($id_devisi=false)
	{
		$ndata=$_POST;
		$data_devisi=$this->master_model_absensi->save_delegated($ndata);
			if($data_devisi){
				$data['result']="Data Berhasil Dihapus";
				$data['status']	= true;
				$data['is_logged_in']=true;
		...
		public function edit_delegated_action($id_devisi=false)
	{
		$ndata=$_POST;
		$data_devisi=$this->master_model_absensi->edit_delegated($ndata);
			if($data_devisi){
				$data['result']="Data Berhasil Dihapus";
				$data['status']	= true;
				$data['is_logged_in']=true;
		...
		public function get_data_delegated($id_user=false){
			$data['data']=$this->master_model_absensi->delegated_set($id_user);
		for($i=0;$i<count($data['data']);$i++){
				
				$data['data'][$i]['nama_user']=$this->master_model_user->data_master_user($data['data'][$i]['id']);
		...
		public function get_data_delegated_view($id_user=false){
			$data['data']=$this->master_model_absensi->delegated_set_view($id_user);
		for($i=0;$i<count($data['data']);$i++){
	...
	public function input_delegated($id_user=false)
	{
			$data['data']=$this->master_model_absensi->delegated_set($id_user);
		for($i=0;$i<count($data['data']);$i++){
		...
		$this->load->view('style.php');
		$this->load->view('menu_header.php');
		$this->load->view('page/delegated/input_delegated.php',$data);
		$this->load->view('footer.php');
		//$this->output->set_content_type('application/json')->set_output(json_encode($data));	
	}
	
		public function edit_delegated($id_user=false)
	{
			$data['data']=$this->master_model_absensi->delegated_set_view($id_user);
		for($i=0;$i<count($data['data']);$i++){
		...
		$this->load->view('style.php');
		$this->load->view('menu_header.php');
		$this->load->view('page/delegated/edit_delegated.php',$data);
		$this->load->view('footer.php');
		//$this->output->set_content_type('application/json')->set_output(json_encode($data));	
	}
