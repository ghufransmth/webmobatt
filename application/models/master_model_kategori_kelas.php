<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	class Master_model_kategori_kelas extends CI_Model {
		
		public function data_master_kategori_kelas($id_shoes_category=false){
			if($id_shoes_category!=false){
				$this->db->where('id_kelas',$id_shoes_category);
			}

			$this->db->where('deleted',0);
			$get_query=$this->db->get('master_data_kelas');
			if($get_query){
		...
		public function save($data){
				...
				$data_barang['pendidikan']=$data['pendidikan'];
				$data_barang['nama_kelas']=$data['nama_kelas'];
				$data_barang['id_user']=$data['id_user'];
				
				$insert=$this->db->insert('master_data_kelas',$data_barang);
		...
		public function edit($data_set){
				...
				//$data['gambar']=$file_profil_save;
					$data_barang['pendidikan']=$data_set['pendidikan'];
					$data_barang['nama_kelas']=$data_set['nama_kelas'];
					$data_barang['id_user']=$data_set['id_user'];
					
					$this->db->where('id_kelas',$data_set['id_kelas']);
					$update=$this->db->update('master_data_kelas',$data_barang);
		...
		public function delete_data_kategori_kelas($id_shoes_category){
			
		
			$query=$this->db->query('update master_data_kelas set deleted="1" WHERE id_kelas="'.$id_shoes_category.'"');
