								...
								<div class="form-group">
									<label for="exampleInputEmail1">Delegated Date</label>
									  <div class='input-group date' id='jam_masuk'>
										<input type='text' class="form-control" name="delegated_date" id="start_date"readonly />
									...
									<label for="exampleInputEmail1">Undelagated Date</label>
									  <div class='input-group date' id='jam_keluar'>
										<input type='text' class="form-control" name="undelegated_date" id="start_date"readonly />
										<span class="input-group-addon">
										  <span class="glyphicon glyphicon-calendar"></span>
										</span>
...
<script>
	$('#form_input_shoes').submit(function(e){
		...
		formData = new FormData($(this)[0]);
			$.ajax({
					url 	: '<?php echo base_url() ?>/index.php/delegated/save_delegated',
					type	: 'POST',
					data	: formData,
					async	: false,
					cache	: false,
...
<script>
            ...
            $.ajax({
                url     : '<?php echo base_url() ?>index.php/delegated/get_data_delegated',
                type    : 'GET',
                async   : false,
                cache   : false,
                processData : false,
                ...
                success:function (data_master_equipment){
						$('#id_pegawai').append('<option value="">--Select Pegawai--</option>');
